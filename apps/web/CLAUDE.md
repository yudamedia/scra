...
16. **Better Auth's `search_path=auth,public` connection option (gotcha #3) breaks over a pooled/pgbouncer Postgres connection** — e.g. Neon's default pooled connection string. Postgres startup parameters like `-c search_path=...` aren't supported through pgbouncer; Neon rejects the connection outright with `unsupported startup parameter in options: search_path`, which surfaces as a 500 on every Better Auth route (magic-link login, the portal-test-login endpoint, etc.) with no build-time warning — this bit production once already (`/api/auth/portal-test-login` 500ing while working fine locally against plain Docker Postgres, which has no pooler). Better Auth has no alternative "schema" option for a raw `pg.Pool` — `search_path` is the only mechanism it supports, so the fix is to stop routing that specific connection through the pooler: `src/lib/auth.ts` now reads `AUTH_DATABASE_URI` (falling back to `DATABASE_URI` if unset) for its `Pool`, and production/preview on Vercel must set `AUTH_DATABASE_URI` to the database's **unpooled/direct** connection string (Neon exposes both). Payload's own `DATABASE_URI` connection is unaffected and can stay pooled, since Payload doesn't set this option.
